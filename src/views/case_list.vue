...
<div class="search-result-list">
  ...
  <a-table :columns="columns" :data-source="data" :scroll="{ x: 1900, y: 600 } " :pagination="{total:total }"  @change="onChange">
...
export default defineComponent({
  ...
  setup(){
    ...
    const copy=(record)=>{
      ...
      delete record['project__name']
      cases_create(record).then((res) => {
               if (res.code == 200)
               {
                   message.success({
                           content: "复制成功！",
...
const optionsProject = ref([])
    ...
    project_get().then((res) => {
        ...
        for (var i=0;i<(res.data).length;i++){
          ...
          if ( par.project_id ==res.data[i].id ){formState.project_id = res.data[i].id}
        }
